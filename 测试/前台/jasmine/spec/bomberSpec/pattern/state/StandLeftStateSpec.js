describe("StandLeftState.js", function () {
    ...
    describe("stand", function () {
        ...
        it("context.sprite的动画为stand_left", function () {
            spyOn(context.sprite, "runAnim");

            state.stand();

            expect(context.sprite.runAnim).toHaveBeenCalledWith("stand_left");
        });
    ...
    describe("walkLeft", function () {
        it("重置当前帧为0", function () {
            spyOn(state.P_context.sprite, "resetCurrentAnim");

            state.walkLeft();

            expect(state.P_context.sprite.resetCurrentAnim).toHaveBeenCalledWith(0);
        });
