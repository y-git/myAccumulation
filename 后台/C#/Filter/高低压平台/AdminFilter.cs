...
namespace BLL
{
    public class AdminAttributeFilter :　ActionFilterAttribute
    {
        ...
        public override void  OnActionExecuting(ActionExecutingContext filterContext)
        {
            //bool a = isJson;
            //bool b = IsJson;
            if (filterContext.HttpContext.Session["userID"] == null || !Convert.ToString(filterContext.HttpContext.Session["roleName"]).Contains("管理员"))
            {
/**无效！估计是因为跳转后的页面的HttpContext与此行的HttpContext（即跳转前页面的HttpContext）不同
                ...
                if (filterContext.HttpContext.Request.IsAjaxRequest())
                {
                    ...
                    else
                    {
                        ...
                        filterContext.Result = new AdminAjaxUnauthorizedResult();
                    }
                }
                else
                {
                    filterContext.Result = new RedirectToRouteResult("default", new RouteValueDictionary(new { controller = "Account", action = "AdminLoginIndex" }));
                }
                //base.OnActionExecuting(filterContext);  //已经跳转了，所以不行要此行代码
            }
        }
    }
    public class AdminAjaxUnauthorizedResult : JavaScriptResult
    {
        public AdminAjaxUnauthorizedResult()
        {
            var loginurl = "/Account/AdminLoginIndex";  //session为空时ajax（返回的不是json格式的数据）的跳转地址
