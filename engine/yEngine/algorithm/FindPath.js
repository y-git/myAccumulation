...
//    var pass = YE.Config.map.terrain.pass,
//        stop = YE.Config.map.terrain.stop;
...
//    var arr_map_tmp = window.terrainData; //存储从游戏中读入的地形数据
//    var map_w = arr_map_tmp.length;

    function aCompute(terrainData, begin, end) {
        //计算运行时间
        ...
        arr_map = setMap(terrainData);

        map_w = terrainData.length;
        beginx = begin.x;
    ...
    /**********************************************************************
     *function setMap(n)
     *功能：把外部的地形数据抽象成该算法中可操作数组的形式来输入算法
     *参数：n为地图的宽度，生成方阵地图
     ************************************************************************/
    function setMap(terrainData) {
        map_w = terrainData.length;
        var m = map_w * map_w;

        var arr_map0 = new Array(); //该函数对地形数据转换的操作数组
        var a = m - 1;
        for (a; a >= 0; a--) {
            var xTmp = tile_x(a); //把ID 编号值转换为x坐标值，用来对应读入地形数据
            var yTmp = map_w - 1 - tile_y(a); //把ID 编号值转换为y坐标值，用来对应读入地形数据,对应数组标号和我自定义xoy坐标位置

            //[cost,parent,g,h,id]
            if (terrainData[yTmp][xTmp] == pass)
    ...
    function setSuccessorNode(bestNodeNum, n) {
        ...
        if (YE.Config.algorithm.DIRECTION == 8) {
        ...
        if (YE.Config.algorithm.DIRECTION == 4) {
            tmp[0] = [0, 0, 0, 0, 0];
            tmp[2] = [0, 0, 0, 0, 0];
            tmp[5] = [0, 0, 0, 0, 0];
            tmp[7] = [0, 0, 0, 0, 0];
    ...
    function _reset() {
        ...
        arr_map = new Array();
//        arr_map_tmp = window.terrainData;
