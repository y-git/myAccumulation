...
describe("Director", function () {
    ...
    afterEach(function () {
        Director.forTest_clearInstance();
        sandbox.restore();
    });

    describe("该类为单例类", function () {
        testTool.judgeSingleInstance(Director);
    });
    ...
    it("第一次创建实例时，进行初始化", function () {
        ...
        fakeDirector.getInstance = Director.getInstance;
        Director.forTest_clearInstance();
    ...
    describe("runWithScene", function () {
        ...
        it("设置当前场景，获得场景类实例", function () {
            sandbox.spy(director, "setCurrentScene");
        ...
        it("启动主循环", function () {
            spyOn(director, "ye_startLoop");

            director.runWithScene(fakeScene);

            expect(director.ye_startLoop).toHaveBeenCalled();
        });
    });
