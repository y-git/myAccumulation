...
//        }
    ...
    describe("构造函数", function () {
        ...
        it("如果传入画布的id参数，则获得canvas", function () {
            ...
            expect(layer.forTest_getCanvas()).toBeCanvas();
        });
        it("如果传入画布的id参数和position参数，则获得canvas，设置画布position坐标", function () {
            layer = getInstance("t_c", {x: 10, y: 20});

            expect(layer.forTest_getCanvas()).toBeCanvas();
            expect(layer.forTest_getCanvas().style.position).toEqual("absolute");
            expect(layer.forTest_getCanvas().style.top).toEqual("20px");
            expect(layer.forTest_getCanvas().style.left).toEqual("10px");
        });
    ...
    describe("change", function () {
        ...
        it("否则，调用setStateNormal", function () {
            ...
            expect(layer.setStateNormal.calls.length).toEqual(2);
        });
    });
    ...
    describe("ye_P_run", function () {
        ...
        beforeEach(function () {
            ...
            spyOn(layer, "iterator");
        });

        it("执行精灵的onstartLoop方法", function () {
            sandbox.stub(layer, "iterator");

            layer.ye_P_run();

            expect(layer.iterator.calledWith("onbeforeRun")).toBeTruthy();
        });
        it("执行精灵类所有动作", function () {
            layer.ye_P_run();

            expect(layer.iterator).toHaveBeenCalledWith("update");
        });
        ...
        describe("如果_state为change", function () {
            ...
            it("调用精灵类的onbeforeDraw方法", function () {
                ...
                expect(layer.iterator.calls[2].args).toEqual(["onbeforeDraw", layer.getContext()]);
            });
            it("调用draw，传入context", function () {
                var fakeContext = {};
                spyOn(layer, "draw");
                spyOn(layer, "getContext").andReturn(fakeContext);

                layer.ye_P_run();

                expect(layer.draw).toHaveBeenCalledWith(fakeContext);
            });
            it("调用精灵类的onafterDraw方法", function () {
                layer.ye_P_run();

                expect(layer.iterator.calls[4].args).toEqual(["onafterDraw", layer.getContext()]);
            });
        ...
        it("执行精灵的onendLoop方法", function () {
            ...
            expect(layer.iterator.calledWith("onafterRun")).toBeTruthy();
        });
    ...
    describe("getCanvasData", function () {
        it("获得画布数据", function () {
            layer.forTest_setCanvas({width: 1, height: 2});
...
//                });
        ...
        describe("setPosition", function () {
            it("设置position的定位模式，默认为绝对定位", function () {
                ...
                expect(layer.forTest_getCanvas().style.position).toEqual("relative");
            });
            it("设置画布位置", function () {
                layer.setPosition(10, 20);
    ...
    describe("startLoop", function () {
        ...
        it("调用每个sprite的onstartLoop", function () {
            ...
            expect(layer.iterator.calledWith("onstartLoop")).toBeTruthy();
        });
        it("先调用自己的onstartLoop，再调用每个sprite的onstartLoop", function(){
            layer.startLoop();

            expect(layer.iterator.calledAfter(layer.onstartLoop)).toBeTruthy();
        });
    });
    ...
    describe("endLoop", function () {
        ...
        it("调用每个sprite的onendLoop", function () {
            ...
            expect(layer.iterator.calledWith("onendLoop")).toBeTruthy();
        });
        it("先调用每个sprite的onendLoop，再调用自己的onendLoop", function(){
            layer.endLoop();

            expect(layer.iterator.calledBefore(layer.onendLoop)).toBeTruthy();
        });
    });
    ...
    describe("虚方法", function () {
        ...
        describe("clear", function () {
            it("清空画布", function () {
                ...
                layer.forTest_setContext(jasmine.createSpyObj("", ["clearRect"]));

                layer.clear();

                expect(layer.getContext().clearRect).toHaveBeenCalledWith(0, 0, 1, 2);
            });
        });

        describe("draw", function () {
            it("调用每个精灵类的draw方法，传入参数context、canvasWidth、canvasHeight", function () {
                var sprite1 = buildFakeSprite(["draw"]) ,
                    sprite2 = buildFakeSprite(["draw"]);
                setCanvasData(layer, 10, 20);
                layer.forTest_addChilds([sprite1, sprite2]);

                layer.draw();

                expect(sprite1.draw).toHaveBeenCalledWith(layer.getContext());
                expect(sprite2.draw).toHaveBeenCalledWith(layer.getContext());
            });
        });
    });
})
