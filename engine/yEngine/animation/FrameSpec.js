...
describe("Frame", function () {
    ...
    describe("操作bitmap", function () {
        ...
        describe("setAnchor", function () {
            it("设置bitmap锚点", function () {
                fakeBitmap.setAnchor = jasmine.createSpy();

                frame.setAnchor(10, 20);

                expect(fakeBitmap.setAnchor).toHaveBeenCalledWith(10, 20);
            });
        });

        describe("setFlipX", function () {
            it("设置bitmap的x轴翻转", function () {
                fakeBitmap.setFlipX = jasmine.createSpy();

                frame.setFlipX();

                expect(fakeBitmap.setFlipX).toHaveBeenCalled();
            });
        });

        describe("setFlipY", function () {
            it("设置bitmap的y轴翻转", function () {
                fakeBitmap.setFlipY = jasmine.createSpy();

                frame.setFlipY();

                expect(fakeBitmap.setFlipY).toHaveBeenCalled();
            });
        });
    ...
    describe("copy", function () {
        it("返回副本，共享rect，但是获得bitmap的副本", function () {
            var bitmapCopy = {a: 1};
            var fakeBitmap = testTool.spyReturn("copy", bitmapCopy);
            var fakeRect = {b: 1};
